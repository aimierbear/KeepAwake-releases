...
1. 从[最新 release](../../releases/latest) 下载 `KeepAwake-0.3.0.dmg`
...
### 🌡️ v0.3 新增：被动过热保护（默认开启）

开启「合盖不休眠」期间，KeepAwake 会监听系统热压力（`NSProcessInfo.thermalState`）：

- `Serious` 持续 60 秒 → 自动关闭「合盖不休眠」，让系统休眠降温
- `Critical` → 立即关闭

触发后会弹出通知，菜单顶部也会显示提示横幅。你可以在菜单底部偏好区的「合盖时过热保护」关闭（**不推荐**）。

**再次提醒：软件被动保护不能替代物理散热**，仍然要避免「合盖 + 高负载 + 堵住通风口」三件事同时发生。
