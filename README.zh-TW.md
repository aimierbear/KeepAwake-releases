...
1. 從[最新 release](../../releases/latest) 下載 `KeepAwake-0.3.0.dmg`
...
### 🌡️ v0.3 新增：被動過熱保護（預設啟用）

開啟「闔蓋不休眠」期間，KeepAwake 會監聽系統熱壓力（`NSProcessInfo.thermalState`）：

- `Serious` 持續 60 秒 → 自動關閉「闔蓋不休眠」，讓系統休眠降溫
- `Critical` → 立即關閉

觸發後會彈出通知，選單頂部也會顯示提示橫幅。你可以在選單底部偏好區的「闔蓋時過熱保護」關閉（**不建議**）。

**再次提醒：軟體被動保護不能替代物理散熱**，仍然要避免「闔蓋 + 高負載 + 堵住通風口」三件事同時發生。
